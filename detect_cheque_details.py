...
IMAGE_PATH = os.path.join(paths['IMAGE_PATH'], 'test', 't1.png')
...
def getMICR(PATH):
    ...
    mcr_image = test_img[335:360, 45:500]

    #plt.imshow(mcr_image)
    #plt.show()

    #read string values and convert to mcr
    test = pytesseract.image_to_string(mcr_image, lang='micr', config=tessdata_dir_config)
...
try:
    ...
    ExtractionResults["logo"] = getPrintedText(roi)
...
try:
    ...
    ExtractionResults["Address"] = getPrintedText(roi)
